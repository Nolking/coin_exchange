...
import { ArrowDownUp, Settings, Wallet, Loader2, Moon, Sun } from 'lucide-react';
...
const SwapCard: React.FC<SwapCardProps> = ({ addToast, theme, toggleTheme }) => {
  // Data State
  ...
  // Calculation Logic
  const calculateToAmount = useCallback((amount: string, source: Token | null, target: Token | null) => {
    if (!amount || !source || !target || isNaN(parseFloat(amount))) return '';
    const val = parseFloat(amount);
    const result = (val * source.price) / target.price;
    // Show precision up to 8 decimal places for small amounts, stripping trailing zeros
    return parseFloat(result.toFixed(8)).toString(); 
  }, []);

  const calculateFromAmount = useCallback((amount: string, source: Token | null, target: Token | null) => {
    if (!amount || !source || !target || isNaN(parseFloat(amount))) return '';
    const val = parseFloat(amount);
  ...
  const handleFromTokenSelect = (token: Token) => {
    setFromToken(token);
    // If selected token is same as other side, swap them or clear
  ...
  const handleSwap = async () => {
    if (!fromToken || !toToken || !fromAmount || parseFloat(fromAmount) <= 0) {
      addToast({ type: 'error', message: 'Please enter a valid amount to swap.' });
      return;
    }
  ...
  const formatRate = (rate: number) => {
      if (rate === 0) return '0';
      // If rate is very small (e.g., 0.0000123), show significantly more decimals
      if (rate < 0.0001) {
          // Find first non-zero digit index roughly
          const str = rate.toFixed(20);
          const firstNonZero = str.match(/[1-9]/);
          if (firstNonZero && firstNonZero.index) {
             return rate.toFixed(firstNonZero.index - 1); // rough precision adjustment
          }
  ...
  const exchangeRate = fromToken && toToken 
    ? `1 ${fromToken.symbol} = ${formatRate(fromToken.price / toToken.price)} ${toToken.symbol}` 
    : '-';
  ...
  if (loading) {
    return (
        <div className="w-full max-w-md bg-white dark:bg-dark-card rounded-3xl p-8 border border-gray-200 dark:border-dark-border shadow-2xl flex flex-col items-center justify-center min-h-[400px]">
            ...
            <span className="text-gray-500 dark:text-gray-400 font-medium">Loading Prices...</span>
        </div>
    )
  }
    ...
    <>
      <div className="w-full max-w-md bg-white dark:bg-dark-card rounded-3xl p-4 border border-gray-200 dark:border-dark-border shadow-2xl relative transition-colors duration-300">
        ...
        <div className="flex justify-between items-center mb-4 px-2 relative">
          <h2 className="text-gray-900 dark:text-white font-semibold text-lg">Swap</h2>
          <div ref={settingsRef}>
            <button 
            ...
            {isSettingsOpen && (
                <div className="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 z-50 overflow-hidden animate-fade-in-up">
                    <div className="p-4">
                        <h4 className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-3">Theme</h4>
                        <div className="flex items-center justify-between">
                            <span className="text-sm text-gray-700 dark:text-gray-300 flex items-center gap-2">
                             ...
                             <button 
                                onClick={toggleTheme}
                                className={`w-11 h-6 flex items-center rounded-full p-1 transition-colors duration-300 focus:outline-none ${theme === 'dark' ? 'bg-primary' : 'bg-gray-300'}`}
                            >
                                <div 
                                    className={`bg-white w-4 h-4 rounded-full shadow-md transform duration-300 ease-in-out ${theme === 'dark' ? 'translate-x-5' : 'translate-x-0'}`}
                                ></div>
                            </button>
                        </div>
                    </div>
        ...
        <div className="bg-light-input dark:bg-dark-input rounded-2xl p-4 mb-2 hover:border hover:border-gray-300 dark:hover:border-gray-600 border border-transparent transition-all">
          <div className="flex justify-between mb-2">
            <span className="text-gray-500 dark:text-gray-400 text-sm font-medium">Sell</span>
            <span className="text-gray-500 dark:text-gray-400 text-sm">Balance: 0.00</span> 
          </div>
          ...
          <div className="flex justify-between mt-2 h-5">
             <span className="text-gray-400 dark:text-gray-500 text-xs">
                 {fromToken && fromAmount ? `$${(parseFloat(fromAmount) * fromToken.price).toFixed(2)}` : ''}
             </span>
          </div>
        </div>
        ...
        <div className="bg-light-input dark:bg-dark-input rounded-2xl p-4 mt-2 mb-4 hover:border hover:border-gray-300 dark:hover:border-gray-600 border border-transparent transition-all">
          <div className="flex justify-between mb-2">
            <span className="text-gray-500 dark:text-gray-400 text-sm font-medium">Buy</span>
             <span className="text-gray-500 dark:text-gray-400 text-sm">Balance: 0.00</span>
          </div>
          <div className="flex items-center gap-3">
            <input 
           ...
           <div className="flex justify-between mt-2 h-5">
             <span className="text-gray-400 dark:text-gray-500 text-xs">
                 {toToken && toAmount ? `$${(parseFloat(toAmount) * toToken.price).toFixed(2)}` : ''}
             </span>
          </div>
        </div>
